...
import {
  ...
  Activity, Beaker, Eye, FileText, Loader2
} from "lucide-react";
import type { InstrumentFile } from "../types";
import ActionLog from "./ActionLog";
import { INSTRUMENT_FILES, ANALYZE_LOG_ENTRIES } from "../data/mock-pipeline";
import type { AnalysisBatchResult, AnalysisFileResult } from "../lib/api";
...
function StatusBadge({ status }: { status: InstrumentFile["status"] }) {
  const config = {
  ...
  const { icon: Icon, text, cls } = config[status];
  return (
...
export default function AnalyzeView({ productSmiles, expectedMw }: Props) {
  ...
  // Use live results if available, else mock data
  const displayFiles = liveResults || INSTRUMENT_FILES;
  ...
  const handleFileUpload = async (files: FileList | null) => {
    ...
    try {
      ...
      console.warn("Live analysis failed, falling back to demo:", e);
      setIsUploading(false);
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-zinc-500">Auto-ingest, interpret, and report LCMS / HPLC / NMR data</p>
          </div>
          {!hasAnalyzed && !isAnalyzing && !isUploading && (
            <button onClick={handleAnalyze} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-medium transition-colors">
              <Activity className="w-4 h-4" /> Process Inbox
            </button>
          )}
        </div>

        {/* Live data indicator */}
        {liveResults && hasAnalyzed && (
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-xs text-emerald-400">
            <CheckCircle2 className="w-3.5 h-3.5" /> Live analysis from uploaded files &bull; {liveResults.length} files processed
          </div>
        )}
        ...
        {hasAnalyzed && (
          <div className="flex items-center gap-6 px-4 py-2.5 rounded-lg bg-zinc-900/50 border border-zinc-800 text-xs">
            <div><span className="text-zinc-500">Time from run &rarr; interpreted result:</span> <span className="text-emerald-400 font-semibold">6 minutes</span> <span className="text-zinc-600">(vs. 1&ndash;2 days manual)</span></div>
            <div><span className="text-zinc-500">Files processed:</span> <span className="text-zinc-300 font-semibold">{displayFiles.filter((f: any) => f.status !== "pending").length}/{displayFiles.length}</span></div>
          </div>
        )}

        {/* Drop zone */}
        {!isAnalyzing && !hasAnalyzed && !isUploading && (
          <div
        ...
        {/* Analysis animation (mock path) */}
        {isAnalyzing && (
          <ActionLog entries={ANALYZE_LOG_ENTRIES} running={true} onComplete={() => { setIsAnalyzing(false); setHasAnalyzed(true); }} />
        )}
                    ...
                    <div className="px-4 pb-4 space-y-3 animate-fade-in">
                      ...
                      {interp.impurities.length > 0 && (
                        <div className="bg-zinc-950 rounded-lg p-3 border border-zinc-800">
                          ...
                          <div className="space-y-1.5">
                            {interp.impurities.map((imp: any, i: number) => (
                      ...
                      {interp.anomalies.length > 0 && (
                        <div className="bg-red-950/30 rounded-lg p-3 border border-red-500/20">
                          ...
                          <div className="space-y-1.5">
                            {interp.anomalies.map((a: string, i: number) => (
                      ...
                      <div className="flex justify-end">
                        <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-zinc-700 text-zinc-300 hover:bg-zinc-800 text-xs transition-colors">
                          <Eye className="w-3.5 h-3.5" /> Publish to ELN
                        </button>
                      </div>
                    </div>
                  )}
